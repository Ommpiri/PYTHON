        ...
        {items.map((_, idx) => (
          <div
            ...
            className={`text-xs text-center py-2 rounded ${
              idx === i ? "bg-amber text-primary-foreground font-bold" : "bg-secondary text-muted-foreground"
            }`}
          >
            n={idx + 1}
          </div>
    ...
    <div className="font-mono text-sm grid md:grid-cols-2 gap-4">
      <div>
        ...
        <div className="flex flex-wrap gap-1 mb-2 min-h-[2rem]">
          {list.map((x, i) => (
            <span key={i} className="px-2 py-1 rounded bg-amber/20 text-foreground border border-amber/40">
              [{i}]={x}
            </span>
          ))}
        </div>
        <div className="flex flex-wrap gap-1">
          <button
            ...
            onClick={() => { const n = (list.at(-1) ?? 0) + 1; setList([...list, n]); push(`list.append(${n})`); }}
          >append</button>
          <button
            className="text-xs px-2 py-1 rounded border border-border text-foreground hover:border-amber transition-colors"
            onClick={() => { if (!list.length) return; const v = list[list.length - 1]; setList(list.slice(0, -1)); push(`list.pop() -> ${v}`); }}
          >pop</button>
          <button
            className="text-xs px-2 py-1 rounded border border-border text-foreground hover:border-amber transition-colors"
            onClick={() => { setList([99, ...list]); push("list.insert(0, 99)"); }}
          >insert(0, 99)</button>
        </div>
      </div>
      <div>
        ...
        <div className="flex flex-wrap gap-1">
          <button
            className="text-xs px-2 py-1 rounded bg-teal text-warm-off"
            onClick={() => { const k = "user" + (Object.keys(dict).length + 1); setDict({ ...dict, [k]: Object.keys(dict).length + 1 }); push(`dict["${k}"] = ...`); }}
          >add key</button>
          <button
            className="text-xs px-2 py-1 rounded border border-border text-foreground hover:border-amber transition-colors"
            onClick={() => { const key = Object.keys(dict)[0]; if (!key) return; const cp = { ...dict }; delete cp[key]; setDict(cp); push(`del dict["${key}"]`); }}
          >del first</button>
          <button
            className="text-xs px-2 py-1 rounded border border-border text-foreground hover:border-amber transition-colors"
            onClick={() => { const key = Object.keys(dict)[0] ?? "missing"; push(`dict.get("${key}") -> ${dict[key] ?? "None"}`); }}
          >get</button>
        </div>
      </div>
      <div className="md:col-span-2">
        ...
        <div className="bg-secondary rounded p-2 text-xs leading-6 min-h-[5rem]">
          {log.length === 0 ? <span className="text-muted-foreground">no operations yet</span> :
