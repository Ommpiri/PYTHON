    ...
    <div className="relative w-full max-w-[400px] h-[320px] bg-warm-black border border-white/10 rounded-md overflow-hidden font-mono mx-auto">
      <svg width="100%" height="100%" viewBox="0 0 400 320">
            ...
            <g key={i}>
              ...
              {e.label && d && (
                <text
                  ...
                  fill="currentColor"
                  className="text-teal/70 text-[10px]"
                >
                  {e.label}
                </text>
              )}
        ...
        {layout.nodes.map((n) => {
          ...
          if (isActive) {
            boxClasses +=
              "fill-[#0B1528] stroke-amber stroke-[2px] filter drop-shadow-[0_0_8px_rgba(245,158,11,0.5)] cursor-pointer";
            ...
            boxClasses +=
              "fill-[#0B1528] stroke-amber/40 stroke-[1px] cursor-pointer hover:stroke-amber/70";
        ...
        <div className="absolute inset-0 bg-black/50 flex items-center justify-center p-4">
          <div className="bg-warm-black border border-amber p-4 rounded shadow-xl flex flex-col gap-2">
            ...
            <input
              ...
              type="text"
              value={
                editedLabels[editingNode] ??
                layout.nodes.find((n) => n.id === editingNode)?.label ??
                ""
              }
              onChange={(e) =>
                setEditedLabels((prev) => ({ ...prev, [editingNode]: e.target.value }))
              }
            ...
            <button
              ...
              className="text-xs bg-secondary text-white px-2 py-1 rounded mt-2 hover:bg-amber hover:text-black"
            >
              Save
            </button>
          </div>
